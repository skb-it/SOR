...
int main(){
    ...
    while(1){
        ...
        if(msgrcv_pat_reg == -1){
            perror("[registration.c] error: msgrcv_pat_reg");
            exit(1);
        }
        ...
        if(msg_send_reg_doc == -1){
            perror("[registration.c] error: msg_send_reg_doc");
            exit(1);
        }
        sleep(5);
